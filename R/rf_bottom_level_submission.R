# This is my original solution that forecasts each product individually.
# Can be done entirely in memory on a sufficiently large machine (e.g. m5.24xlarge EC2)
...
sample_df <- read_csv("data/sales_train_evaluation.csv") %>%
  mutate(store_item = paste(item_id, store_id, sep = "_")) %>%
  inner_join(sample_items, by = "store_item") %>%
